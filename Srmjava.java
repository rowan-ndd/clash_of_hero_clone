...
public class Srmjava extends Applet
{
	...
	{
		System.out
				.println("Welcome to SRMj (Frank Nielsen - May 2006) ! Initializing the applet.");
	...
	void OneRound()
	{
		...
		ImageProducer ip = new MemoryImageSource(pg.getWidth(), pg.getHeight(),
				rastero, 0, pg.getWidth());
		...
		AdjustmentListener
{
	...
	private void getGradientMat() throws IOException
	{
		...
		/*
		BufferedImage bufferedImage = new BufferedImage(applet.h,applet.w,BufferedImage.TYPE_INT_RGB);
		...
		for (int i = 0; i < applet.h; i++) // for each row
		{
			for (int j = 0; j < applet.w; j++) // for each column
			{
				...
				{
					int index = i * applet.w + j;
					float grad = (float)Math.tan(gradient[index]);
					//graphics.drawLine(j,i,j+(int)(6.f), i+(int)(6.f*grad));
					//graphics.drawLine(j,i,j+1,i+1);
